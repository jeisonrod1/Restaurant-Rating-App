...
class SearchUserView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if paramstype == "restaurants":
            queryset = Restaurants.objects.filter(name__icontains=params).order_by('-date_created')
            return queryset

        elif paramstype == "reviews":
            queryset = RestaurantsReviews.objects.filter(text_content__icontains=params).order_by('-date_created')
            return queryset
